...
for k=1:(nsteps-i)
    porb = reshape(preds(:,k),[],2);
    plot(x_star,porb(:,1),'k')
    ylim([-max(unext)-1,max(unext)+1])
    pause(.01);
end

disp(size(preds))
disp(size(u_star(:,i:end)))
disp(size(t_star(i:end)))

fig=figure();
%set(fig,'units','normalized','outerposition',[0 0 1 .5])
subplot(3,2,1:2)
plot_surface(t_star((i+2):end), x_star, u_star(:,(i+2):end), '$t$', '$x$', 'Simulated $u(t,x)$');

subplot(3,2,3:4)
plot_surface(t_star((i+2):end), x_star, preds(1:512,:), '$t$', '$x$', 'Approximated $u(t,x)$');

subplot(3,2,5:6)
plot_surface(t_star((i+2):end), x_star, abs(preds(1:512,:)-u_star(:,(i+2):end)), '$t$', '$x$', 'Error');
