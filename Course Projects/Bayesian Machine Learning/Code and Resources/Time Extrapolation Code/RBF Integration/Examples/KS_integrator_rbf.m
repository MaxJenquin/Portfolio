...
% a KS system, with a base RBF kernel then integrates the system in time

function KS_integrator_rbf()
%% Setup
clc; close all;
...
% choose sample time in order to begin before the chaotic regime's onset
s=61;
...
if clean
    ...
    if ~noisy
        save('../Results/rbfKScleanresults.mat','-struct','results');
    end
...
if noisy
    ...
    if ~clean
        ...
        results.simdat = simdat;
        save('../Results/rbfKSnoisyresults.mat','-struct','results');
    else
        results.noisypreds = preds;
        results.noisyvars = vars;
        results.noisyhyps = model.hyp;
        save('../Results/rbfKSresults.mat','-struct','results');
    end
