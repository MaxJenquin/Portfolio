...
% spectral mixture kernel for the KdV equation (flag = 0). For nonzero 
...
switch flag
    ...
    case 1
        ...
        for i=1:nmix
            ...
            sinterm = sin(b.*tau);
            %compute contributions from each derivative of k
            d1k_contr = dt.*(ubar - ubarp).*expterm.*(d1k_cos.*tau.*costerm...
                +d1k_sin.*sinterm);
            d2k_contr = -2*(dt^2)*lambda(1).*ubar.*ubarp.*expterm.*((d2k_cos(1).*tau_2...
                +d2k_cos(2)).*costerm + (d2k_sin(1).*tau).*sinterm);
            d4k_contr = -(dt^2)*lambda(2)*(ubar+ubarp).*expterm.*((d4k_cos(1)...
                .*tau_4+d4k_cos(2).*tau_2+d4k_cos(3)).*costerm + (d4k_sin(1).*tau_3...
                +d4k_sin(2).*tau).*sinterm);
            K = K + weights(i).*(d1k_contr+d2k_contr+d4k_contr);
        end
    case 2
        ...
        for i=1:nmix
            ...
            sinterm = sin(b.*tau);
            %compute contributions from each derivative of k
            d4k_contr = -(dt^2)*lambda(1)*(ubar+ubarp).*expterm.*((d4k_cos(1)...
                .*tau_4+d4k_cos(2).*tau_2+d4k_cos(3)).*costerm + (d4k_sin(1).*tau_3...
                +d4k_sin(2).*tau).*sinterm);
            d6k_contr = -2*(dt^2)*lambda(2).*expterm.*((d6k_cos(1).*tau_6 + d6k_cos(2).*tau_4...
                +d6k_cos(3).*tau_2+d6k_cos(4)).*costerm + (d6k_sin(1).*tau_5 +...
                d6k_sin(2).*tau_3 + d6k_sin(3).*tau).*sinterm);
            K = K + weights(i).*(d4k_contr+d6k_contr);
        end
    otherwise
