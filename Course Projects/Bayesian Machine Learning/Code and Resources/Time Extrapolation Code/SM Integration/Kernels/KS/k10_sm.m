...
switch flag
    case 0
        ...
        for i=1:nmix
            ...
            d1k_contr = (-dt*lambda(1)).*ubarp.*expterm.*(d1k_cos.*tau.*costerm...
                +d1k_sin.*sinterm);
            d2k_contr = (dt*lambda(2)).*expterm.*((d2k_cos(1).*tau_2+d2k_cos(2))...
                .*costerm + d2k_sin(1).*tau.*sinterm);
            d4k_contr = dt*lambda(3)*expterm.*((d4k_cos(1).*tau_4+d4k_cos(2).*tau_2...
                +d4k_cos(3)).*costerm + (d4k_sin(1).*tau_3+d4k_sin(2).*tau).*sinterm);
            K = K + weights(i) * (d0k_contr+d1k_contr+d2k_contr+d4k_contr);
        end
    case 1
    ...
    case 2
        ...
        for i=1:nmix
            ...
            d2k_contr = dt.*expterm.*((d2k_cos(1).*tau_2+d2k_cos(2))...
                .*costerm + (d2k_sin(1).*tau_2).*sinterm);
            K = K + weights(i) * d2k_contr;
        end
    case 3
        ...
        for i=1:nmix
            ...
            d4k_contr = dt.*expterm.*((d4k_cos(1).*tau_4+d4k_cos(2).*tau_2...
                +d4k_cos(3)).*costerm + (d4k_sin(1).*tau_3+d4k_sin(2).*tau).*sinterm);
            K = K + weights(i) * d4k_contr;
        end
    otherwise
