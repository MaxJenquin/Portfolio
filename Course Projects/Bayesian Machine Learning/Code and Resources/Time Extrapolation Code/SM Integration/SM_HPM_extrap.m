...
classdef SM_HPM_extrap
    ...
    methods
        ...
        function obj = train(obj, n_iter, n_restarts)
            ...
            % Note that because some settings result in a singular
            % covariance matrix, we restart the trial if that occurs.
            k=1;
            nfail=0;
            while k<n_restarts
                ...
                try
                    ...
                    fprintf('%d failures, %d successes\n',nfail,k-1)
                end
            end
            % train best result once more for fine tuning, a little longer
            [obj.hyp,~,~] = minimize(best_hyp, @obj.likelihood, -4*n_iter);
